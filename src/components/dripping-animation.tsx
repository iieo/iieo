...
}> = ({ width = window.innerWidth, height = window.innerHeight, backgroundColor = '#0c0c0c' }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numStrokes; i++) {
      ...
      for (let d = 0; d < dripPoints; d++) {
        ...
        drip.position.set(x + offsetX, -height / 2, 0.01);
    ...
    for (let h = 0; h < horizDripCount; h++) {
      ...
      drip.position.set(offsetX, 0.2 - horizHeight / 2, 0.02);
    ...
    const animate = () => {
        ...
        if (drip.active) {
          ...
          if (positionAttr) {
            ...
            for (let j = 0; j < positions.length; j += 3) {
              ...
              positions[j] =
                (positions[j] ?? 0) +
                Math.sin(normalizedY * drip.oscillationFrequency + drip.timeSinceLastDrop * 10) *
          ...
          if (
            drip.timeSinceLastDrop > drip.nextDropTime &&
            ...
            const threadMaterial = (
              dripMaterial ?? inkShaderMaterial
            ).clone() as CustomShaderMaterial;
      ...
      for (let i = threads.length - 1; i >= 0; i--) {
        ...
        thread.mesh.geometry = new THREE.PlaneGeometry(geometry.parameters.width, newHeight, 2, 8);
      ...
      for (let i = droplets.length - 1; i >= 0; i--) {
        ...
        droplet.mesh.position.x +=
          Math.sin(droplet.age * droplet.oscillationSpeed) *
          droplet.oscillationAmount *
          oscillationDamping;
